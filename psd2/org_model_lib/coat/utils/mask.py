...
class exchange_token:
    ...
    def __call__(self, features, mask_box):
        ...
        features[:, mask_x1:mask_x2, mask_y1:mask_y2, :] = features[
            new_idx, mask_x1:mask_x2, mask_y1:mask_y2, :
        ]
...
class jigsaw_token:
    ...
    def __call__(self, features):
        ...
        feature_random = torch.cat(
            [
                features[:, self.begin - 1 + self.shift :, :],
                features[:, self.begin - 1 : self.begin - 1 + self.shift, :],
            ],
            dim=1,
        )
        x = feature_random
...
class exchange_patch:
    ...
    def xpatch_hstripe(self, features, mask_size):
        ...
        for i in range(num_masks):
            ...
            mask_y2 = mask_y1 + mask_size
            new_idx = torch.randperm(features.shape[0])
            features[:, :, :, mask_y1:mask_y2] = features[
                new_idx, :, :, mask_y1:mask_y2
            ]
        return features
    ...
    def xpatch_vstripe(self, features, mask_size):
        ...
        for i in range(num_masks):
            ...
            mask_x2 = mask_x1 + mask_size
            new_idx = torch.randperm(features.shape[0])
            features[:, :, mask_x1:mask_x2, :] = features[
                new_idx, :, mask_x1:mask_x2, :
            ]
        return features
    ...
    def __call__(self, features):
        ...
        if random.random() < 0.5:
            ...
            for i in range(num_masks):
                ...
                mask_y2 = mask_y1 + self.mask_size
                for k in range(features.shape[0]):
                    features[k, :, :, mask_y1[k] : mask_y2[k]] = torch.normal(
                        mean.repeat(dim, 14, 2), std.repeat(dim, 14, 2)
                    )
        else:
            x1_max = features.shape[3] - self.mask_size
            num_masks = 1
            for i in range(num_masks):
                ...
                mask_x2 = mask_x1 + self.mask_size
                for k in range(features.shape[0]):
                    features[k, :, mask_x1[k] : mask_x2[k], :] = torch.normal(
                        mean.repeat(dim, 2, 14), std.repeat(dim, 2, 14)
                    )
    ...
    def __call__(self, features):
        ...
        if random.random() < 0.5:
            ...
            for i in range(num_masks):
                ...
                mask_y2 = mask_y1 + self.mask_size
                new_idx = torch.randperm(features.shape[0])
                features[:, :, :, mask_y1:mask_y2] = (
                    lam * features[:, :, :, mask_y1:mask_y2]
                    + (1 - lam) * features[new_idx, :, :, mask_y1:mask_y2]
                )
        else:
            ...
            for i in range(num_masks):
                ...
                mask_x2 = mask_x1 + self.mask_size
                new_idx = torch.randperm(features.shape[0])
                features[:, :, mask_x1:mask_x2, :] = (
                    lam * features[:, :, mask_x1:mask_x2, :]
                    + (1 - lam) * features[new_idx, :, mask_x1:mask_x2, :]
                )
...
class jigsaw_patch:
    ...
    def __call__(self, features):
        ...
        # Shift Operation
        feature_random = torch.cat(
            [features[:, :, self.shift :], features[:, :, : self.shift]], dim=2
        )
        x = feature_random
