...
class OIMSMR(autograd.Function):
    @staticmethod
    def forward(ctx, inputs, targets, lut, cq, cq_omega, omega_decay, momentum):
        outputs_labeled = inputs.mm(lut.t())
        outputs_unlabeled = inputs.mm(cq.t())

        omega_x_unlabeled = (
            outputs_labeled.max(dim=1)[0] / outputs_unlabeled.max(dim=1)[0]
        )

        ctx.save_for_backward(
            inputs, targets, omega_x_unlabeled, lut, cq, cq_omega, omega_decay, momentum
        )
    ...
    def backward(ctx, grad_outputs):
        ...
        inputs, targets, omega_x_unlabeled = tensor_gather(
