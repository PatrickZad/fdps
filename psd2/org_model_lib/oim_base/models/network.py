...
class Network(nn.Module):
    ...
    def forward(self, img, img_info, gt_boxes, probe_roi=None):
        ...
        if self.training:
            ...
            unlabeled_matching_scores = self.unlabeled_matching_layer(
                features, pid_labels
            )
    ...
    def inference(self, img, probe_roi=None, threshold=0.75):
        ...
        img_info = torch.Tensor(
            [processed_img.shape[2], processed_img.shape[3], scale]
        ).to(device)
        if probe_roi is not None:
            ...
            probe_roi = torch.cat((torch.zeros(1, 1), probe_roi.float()), dim=1).to(
                device
            )
        ...
        stds = (
            torch.Tensor(cfg.TRAIN.BBOX_NORMALIZE_STDS).repeat(num_classes).to(device)
        )
        means = (
            torch.Tensor(cfg.TRAIN.BBOX_NORMALIZE_MEANS).repeat(num_classes).to(device)
        )
