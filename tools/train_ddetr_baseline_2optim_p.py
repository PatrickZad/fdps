...
class Trainer(DefaultTrainer):
    ...
    def build_optimizer(cls, cfg, model):
        ...
        frozen_params = []
        learn_param_keys = []
        param_groups1 = [{"params": [], "lr": cfg.SOLVER.BASE_LR}] + [
            {"params": [], "lr": cfg.SOLVER.BASE_LR * lf}
            for lf in cfg.SOLVER.LR_FACTORS
        ]
        bias_param_groups1 = [{"params": [], "lr": cfg.SOLVER.BASE_LR,"weight_decay":cfg.SOLVER.WEIGHT_DECAY_BIAS}] + [
            {"params": [], "lr": cfg.SOLVER.BASE_LR * lf,"weight_decay":cfg.SOLVER.WEIGHT_DECAY_BIAS}
            for lf in cfg.SOLVER.LR_FACTORS
        ]
        param_groups2 = [{"params": [], "lr": cfg.SOLVER2.BASE_LR}] + [
            {"params": [], "lr": cfg.SOLVER2.BASE_LR * lf}
            for lf in cfg.SOLVER2.LR_FACTORS
        ]
        bias_param_groups2 = [{"params": [], "lr": cfg.SOLVER2.BASE_LR,"weight_decay":cfg.SOLVER2.WEIGHT_DECAY_BIAS}] + [
            {"params": [], "lr": cfg.SOLVER2.BASE_LR * lf,"weight_decay":cfg.SOLVER2.WEIGHT_DECAY_BIAS}
            for lf in cfg.SOLVER2.LR_FACTORS
        ]
        freeze_regex = [re.compile(reg) for reg in cfg.SOLVER.FREEZE_PARAM_REGEX]
        lr_group_regex1 = [re.compile(reg) for reg in cfg.SOLVER.LR_GROUP_REGEX]
        opt2_param_regex=[re.compile(reg) for reg in cfg.SOLVER2.KEY_REGEX]
        lr_group_regex2 = [re.compile(reg) for reg in cfg.SOLVER2.LR_GROUP_REGEX]
    ...
    @classmethod
    def build_optimizer2(cls, cfg, model):
        from psd2.solver.build import maybe_add_gradient_clipping
        import re

        logger = logging.getLogger("psd2.trainer")
        assert len(cfg.SOLVER2.KEYS)>0
        frozen_params = []
        learn_param_keys = []
        param_groups = [{"params": [], "lr": cfg.SOLVER2.BASE_LR}] + [
            {"params": [], "lr": cfg.SOLVER2.BASE_LR * lf}
            for lf in cfg.SOLVER2.LR_FACTORS
        ]
        bias_param_groups = [{"params": [], "lr": cfg.SOLVER2.BASE_LR,"weight_decay":cfg.SOLVER2.WEIGHT_DECAY_BIAS}] + [
            {"params": [], "lr": cfg.SOLVER2.BASE_LR * lf,"weight_decay":cfg.SOLVER2.WEIGHT_DECAY_BIAS}
            for lf in cfg.SOLVER2.LR_FACTORS
        ]
        freeze_regex = [re.compile(reg) for reg in cfg.SOLVER2.FREEZE_PARAM_REGEX]
        lr_group_regex = [re.compile(reg) for reg in cfg.SOLVER2.LR_GROUP_REGEX]
        ...
        logger.info("Frozen parameters:\n{}".format("\n".join(frozen_params)))
        logger.info("Training parameters:\n{}".format("\n".join(learn_param_keys)))
        optim = cfg.SOLVER2.OPTIM
        if optim == "SGD":
            return maybe_add_gradient_clipping(cfg, torch.optim.SGD)(
                param_groups,
                lr=cfg.SOLVER2.BASE_LR,
                momentum=cfg.SOLVER2.MOMENTUM,
                nesterov=cfg.SOLVER2.NESTEROV,
                weight_decay=cfg.SOLVER2.WEIGHT_DECAY,
            )
        elif optim == "Adam":
            return maybe_add_gradient_clipping(cfg, torch.optim.Adam)(
                param_groups,
