...
from fdps.layers import ShapeSpec, nonzero_tuple, batched_nms
from fdps.structures import Boxes, ImageList, Instances, pairwise_iou
from fdps.utils.events import get_event_storage
from fdps.utils.registry import Registry
...
class Res5ROIHeads(ROIHeads):
    ...
    def _build_res5_block(cls, cfg):
        ...
        if cfg.MODEL.RESNETS.DEPTH == 34:  # NOTE resnet18 is not supported for now
            ...
            blocks = ResNet.make_stage(
                BottleneckBlock,
                3,
                stride_per_block=[1, 1, 1] if res5_dilation == 2 else [2, 1, 1],
                in_channels=out_channels // 2,
    ...
    def forward_with_given_boxes(
        ...
        if self.mask_on:
            ...
            return instances
@ROI_HEADS_REGISTRY.register()
class Res5HalfROIHeads(Res5ROIHeads):
    @classmethod
    def _build_res5_block(cls, cfg):
        # fmt: off
        stage_channel_factor = 2 ** 3  # res5 is 8x res2
        ...
        if cfg.MODEL.RESNETS.DEPTH == 34:  # NOTE resnet18 is not supported for now
            ...
            blocks = ResNet.make_stage(
                BottleneckBlock,
                2,
                stride_per_block=[1, 1] if res5_dilation == 2 else [2, 1],
                in_channels=out_channels // 2,
                bottleneck_channels=bottleneck_channels,
                out_channels=out_channels,
                num_groups=num_groups,
...
@ROI_HEADS_REGISTRY.register()
class Res5HalfROIHeadsCkpt(Res5HalfROIHeads):
    def _shared_roi_transform(self, features: List[torch.Tensor], boxes: List[Boxes]):
        x = self.pooler(features, boxes)
        return ckpt.checkpoint(self.res5,x) if self.training else self.res5(x)

@ROI_HEADS_REGISTRY.register()
class Res5MinorROIHeads(Res5ROIHeads):
    @classmethod
    def _build_res5_block(cls, cfg):
        # fmt: off
        stage_channel_factor = 2 ** 3  # res5 is 8x res2
        ...
        if cfg.MODEL.RESNETS.DEPTH == 34:  # NOTE resnet18 is not supported for now
            ...
            blocks = ResNet.make_stage(
                BottleneckBlock,
                1,
                stride_per_block=[1,] if res5_dilation == 2 else [2,],
                in_channels=out_channels // 2,
                bottleneck_channels=bottleneck_channels,
                out_channels=out_channels,
                num_groups=num_groups,
...
class StandardROIHeads(ROIHeads):
    ...
    def _init_box_head(cls, cfg, input_shape):
        ...
        if isinstance(pooler_resolution, Iterable):
