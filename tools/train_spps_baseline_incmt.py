...
class IncmtTrainer(DefaultTrainer):
    #     """
    ...
    def __init__(self, cfg, model_find_unused_parameters=True):
        ...
        incmt_cfg = cfg.INCMT
        self.name_domains = cfg.DATASETS.TRAIN + incmt_cfg.DATASETS.TRAIN
...
def main(args):
    ...
    if args.eval_only:
        model = IncmtTrainer.build_model(cfg)
        DetectionCheckpointer(model, save_dir=cfg.OUTPUT_DIR).resume_or_load(
            cfg.MODEL.WEIGHTS, resume=args.resume
        )
        res = IncmtTrainer.test(cfg, model)
        if cfg.TEST.AUG.ENABLED:
            res.update(IncmtTrainer.test_with_TTA(cfg, model))
        if comm.is_main_process():
            verify_results(cfg, res)
        return res

    trainer = IncmtTrainer(cfg, model_find_unused_parameters=not args.n_find_unparams)
