...
def update_prior_dist(batch_size, alpha, beta):
    ...
    """
    global prior_dist
    grid_points = torch.arange(1.0, 2 * batch_size, 2.0).float().cuda() / (
        2 * batch_size
    )
...
def shapingloss(assign, radius, std, num_parts, alpha, beta, eps=1e-5):
    ...
    if radius == 0:
        ...
        weight = (
            ...
            .expand(num_parts, 1, 2 * radius + 1, 2 * radius + 1)
            .cuda()
        )
    ...
    num_as = assign.shape[0]
    num_as = torch.as_tensor([num_as], dtype=torch.float, device=assign.device)

    # For dist only
    comm.synchronize()
    all_num_as = comm.all_gather(num_as)
    num_as = sum([num.to("cpu") for num in all_num_as])
    num_as = torch.clamp(num_as / comm.get_world_size(), min=1).item()
    loss = output_nk.sum() / num_as
    return loss
