...
class Trainer(DefaultTrainer):
    ...
    def build_optimizer(cls, cfg, model):
        ...
        lr_linear_proj_names = [
            "reference_points"
        ]  # TDOD check if "sampling_offsets" refers to decoders' boxembeds
        learnable_p_names = []
        ...
        param_dicts = [
            {
                "params": [
                    ...
                    and not _match_name_keywords(n, lr_linear_proj_names)
                    and not key_in(freeze_keys, freeze_excepts, n)
                    and p.requires_grad
                ],
            ...
            },
            {
                "params": [
                    dumm_save_name(n, p)
                    for n, p in model.named_parameters()
                    if _match_name_keywords(n, lr_backbone_names)
                    and not key_in(freeze_keys, freeze_excepts, n)
                    and p.requires_grad
                ],
                "lr": cfg.DETECTOR.SOLVER.BACKBONE_LR,
            },
            {
                "params": [
                    dumm_save_name(n, p)
                    for n, p in model.named_parameters()
                    if _match_name_keywords(n, lr_linear_proj_names)
                    and not key_in(freeze_keys, freeze_excepts, n)
                    and p.requires_grad
                ],
                "lr": cfg.SOLVER.BASE_LR * cfg.DETECTOR.SOLVER.LINEAR_PROJ_LR_FACTOR,
            },
        ]
    ...
    def run_step(self):
        super().run_step()
        # trivial log for wandb vis gradient
        if comm.is_main_process() and self.log_wandb:
            wandb.log({"trivial_step": self.iter}, step=self.iter)
