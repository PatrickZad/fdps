...
"""
FP DETR model and criterion classes.
"""
import torch
...
from psd2.modeling.transformer import FPDeformableEncTransformer
import copy
from psd2.modeling.reid_heads import build_reid_head, BnneckHead
...
class FPDT_PS_Baseline(SearchBase):
    ...
    def __init__(
        ...
        *,
        ...
        num_pred = (
            (self.transformer.enc_head.num_layers + 1)
            if two_stage
            else self.transformer.enc_head.num_layers
        )
        if with_box_refine:
            ...
            self.transformer.enc_head.bbox_embed = self.bbox_embed
        else:
            ...
            self.transformer.enc_head.bbox_embed = None
        if two_stage:
    ...
    def from_config(cls, cfg):
        ...
        if deep_supervision:
            aux_weight_dict = {}
            for i in range(trans_cfg.HEAD_DEPTH - 1):
    ...
    def forward(self, input_list):
        ...
        """
        det_output = all_output
        spatial_shapes, level_start_index, valid_ratios, mask_flatten = aux_info
        if self.training:
            # det losses
