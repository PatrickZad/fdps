...
import copy
import time
from psd2.modeling.meta_arch.person_search import DDETR_Baseline,DDETR_Baseline_JointDc
...
class TiDDETR_2StreamOnCrops(DDETR_Baseline):
    @configurable
...
@META_ARCH_REGISTRY.register()
class TiDDETR_C4Side2StreamOnCrops(TiDDETR_2StreamOnCrops):
    @configurable
...
@META_ARCH_REGISTRY.register()
class TiDDETR_JointDc_2StreamOnCrops(TiDDETR_2StreamOnCrops,DDETR_Baseline_JointDc):
    def forward(self, input_list):
        return DDETR_Baseline_JointDc.forward(self,input_list)
    def forward_det(self, image_list, features, gt_instances):
        return DDETR_Baseline_JointDc.forward_det(self, image_list, features, gt_instances)
    def get_ps_pos_samples(self,gts):
        return DDETR_Baseline_JointDc.get_ps_pos_samples(self,gts)
...
@META_ARCH_REGISTRY.register()
class TiDDETR_JointDc_C4Side2StreamOnCrops(TiDDETR_JointDc_2StreamOnCrops):
    @configurable
    def __init__(self, bn_neck, side_init, **kwargs) -> None:
        super().__init__(**kwargs)
...
@META_ARCH_REGISTRY.register()
class TiDDETR_C4Side2StreamOnCropsS3Infer(TiDDETR_C4Side2StreamOnCrops):
    @configurable
    def __init__(self, reid_pooler_res2,reid_pooler_res3, **kwargs) -> None:
        super().__init__(**kwargs)
        self.reid_pooler_res2=reid_pooler_res2
        self.reid_pooler_res3=reid_pooler_res3
    @classmethod
    def from_config(cls, cfg):
        res = super().from_config(cfg)
        reid_cfg = cfg.REID_HEAD
        reid_pooler_cfg = reid_cfg.ROI_POOLER
        resolution_2=(reid_pooler_cfg.POOLER_RESOLUTION[0]*4,reid_pooler_cfg.POOLER_RESOLUTION[1]*4)
        scales_2=[sc*4 for sc in reid_pooler_cfg.SCALES]
        res["reid_pooler_res2"] = ROIPooler(
            output_size=resolution_2,
            scales=scales_2,
            sampling_ratio=reid_pooler_cfg.SAMP_RATIO,
            pooler_type=reid_pooler_cfg.TYPE,
            output_channels=256,  #  compatible with deformRoiPool
        )
        resolution_3=(reid_pooler_cfg.POOLER_RESOLUTION[0]*2,reid_pooler_cfg.POOLER_RESOLUTION[1]*2)
        scales_3=[sc*2 for sc in reid_pooler_cfg.SCALES]
        res["reid_pooler_res3"] = ROIPooler(
            output_size=resolution_3,
            scales=scales_3,
            sampling_ratio=reid_pooler_cfg.SAMP_RATIO,
            pooler_type=reid_pooler_cfg.TYPE,
            output_channels=256,  #  compatible with deformRoiPool
    ...
    def get_reid_backbone_features(self, det_backbone_features, image_list):
        ...
        return det_backbone_features["res2"],reid_res3_feat,reid_res4_feat

    def get_reid_person_features(self, reid_backbone_features, person_boxes):
        ms_roi_feats=[]
        res2,res3,res4=reid_backbone_features
        d2_boxes = [Boxes(pi_boxes) for pi_boxes in person_boxes]
        roi_feats = self.reid_pooler([res4], d2_boxes)
        roi_feats = (
            checkpoint.checkpoint(self.side_res5, roi_feats)
            if self.use_checkpoint and self.training
            else self.side_res5(roi_feats)
        )  # n x c x h x w
        ms_roi_feats.append(roi_feats)
        # from res2
        roi_feats = self.reid_pooler_res2([res2], d2_boxes)
        det_res3=self.backbone.stages[1](roi_feats)
        reid_res3=self.side_res3(roi_feats)
        alpha = torch.sigmoid(self.alpha_res3)
        roi_feats = (
            alpha * det_res3+ (1 - alpha) * reid_res3
        )
        det_res4=self.backbone.stages[2](roi_feats)
        reid_res4=self.side_res4(roi_feats)
        alpha = torch.sigmoid(self.alpha_res4)
        roi_feats = (
            alpha * det_res4+ (1 - alpha) * reid_res4
        )
        roi_feats = self.side_res5(roi_feats)
        ms_roi_feats.append(roi_feats)
        # from res3
        roi_feats = self.reid_pooler_res3([res3], d2_boxes)
        det_res4=self.backbone.stages[2](roi_feats)
        reid_res4=self.side_res4(roi_feats)
        alpha = torch.sigmoid(self.alpha_res4)
        roi_feats = (
        ...
        roi_feats = self.side_res5(roi_feats)
        ms_roi_feats.append(roi_feats)
        roi_feats=torch.cat(ms_roi_feats,dim=0)
        return roi_feats
    def get_reid_embed(self, p_feat_maps):
        pfeat_embs = self.pfeat_pooling(p_feat_maps)
        pfeat_embs = pfeat_embs.view(pfeat_embs.shape[0], pfeat_embs.shape[1])
        pfeat_embs = self.bn_neck(pfeat_embs)
        pfeat_embs=pfeat_embs.reshape(3,-1,pfeat_embs.shape[1]).transpose(0,1).flatten(1,2)
        return tF.normalize(pfeat_embs, dim=-1)
    def inf_query(self, input_list):
        ...
        del reid_bk_feats
        q_embs = self.get_reid_embed(torch.split(q_featmaps,1))
        del q_featmaps
...
from psd2.modeling.poolers import ROIPooler
@META_ARCH_REGISTRY.register()
class TiDDETR_C4Side2StreamOnCropsS2Infer(TiDDETR_C4Side2StreamOnCropsS3Infer):
    def get_reid_backbone_features(self, det_backbone_features, image_list):
        del image_list
        reid_res3_feat = (
            checkpoint.checkpoint(self.side_res3, det_backbone_features["res2"])
        ...
        return reid_res3_feat,reid_res4_feat

    def get_reid_person_features(self, reid_backbone_features, person_boxes):
        ms_roi_feats=[]
        res3,res4=reid_backbone_features
        d2_boxes = [Boxes(pi_boxes) for pi_boxes in person_boxes]
        roi_feats = self.reid_pooler([res4], d2_boxes)
        roi_feats = (
            checkpoint.checkpoint(self.side_res5, roi_feats)
    ...
    def get_reid_embed(self, p_feat_maps):
        ...
        pfeat_embs = pfeat_embs.view(pfeat_embs.shape[0], pfeat_embs.shape[1])
        pfeat_embs = self.bn_neck(pfeat_embs)
        pfeat_embs=pfeat_embs.reshape(2,-1,pfeat_embs.shape[1]).transpose(0,1).flatten(1,2)
        return tF.normalize(pfeat_embs, dim=-1)

@META_ARCH_REGISTRY.register()
class TiDDETR_C4Side2StreamOnCropsOimProtoCon(TiDDETR_C4Side2StreamOnCrops):
    @configurable
...
@META_ARCH_REGISTRY.register()
class TiDDETR_C4Side2StreamOnCropsOimProtoConRe(TiDDETR_C4Side2StreamOnCropsOimProtoCon):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args,**kwargs)
...
@META_ARCH_REGISTRY.register()
class TiDDETR_C4Side2StreamOnCropsOimProtoConReFlip(TiDDETR_C4Side2StreamOnCropsOimProtoConRe):
    def get_crops(self,image_list,boxes):
        tgt_size=(256,128)
        crops=[]
        for bi,boxesi in enumerate(boxes):
...
@META_ARCH_REGISTRY.register()
class TiDDETR_JointDc_C4Side2StreamOnCropsOimProtoCon(TiDDETR_JointDc_C4Side2StreamOnCrops):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args,**kwargs)
        self.crop_loss=copy.deepcopy(self.reid_loss)
...
@META_ARCH_REGISTRY.register()
class TiDDETR_JointDc_C4Side2StreamOnCropsOimProtoConRe(TiDDETR_JointDc_C4Side2StreamOnCropsOimProtoCon):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args,**kwargs)
        self.re=RandomErasingAfterNorm(self.pixel_mean[:,0,0],self.pixel_std[:,0,0])
...
import torchvision.transforms.functional as tvF
@META_ARCH_REGISTRY.register()
class TiDDETR_JointDc_C4Side2StreamOnCropsOimProtoConReFlip(TiDDETR_JointDc_C4Side2StreamOnCropsOimProtoConRe):
    def get_crops(self,image_list,boxes):
        tgt_size=(256,128)
        crops=[]
        for bi,boxesi in enumerate(boxes):
...
@META_ARCH_REGISTRY.register()
class TiDDETR_C4Side2StreamOnCropsOimProtoConReFlipTrip(TiDDETR_C4Side2StreamOnCropsOimProtoConReFlip):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args,**kwargs)
...
from .ti_ddetr_baseline import DnDabDDetrDetHead,DabDDetrDetHead
@META_ARCH_REGISTRY.register()
class TiDnDabDDETR_C4Side2StreamOnCropsOimProtoConReFlip(TiDDETR_C4Side2StreamOnCropsOimProtoConReFlip):
    @classmethod
    def from_config(cls, cfg):
        ret = super().from_config(cfg)
        ret["det_head"]=DnDabDDetrDetHead(cfg,ret["backbone"].output_shape())
        return ret
@META_ARCH_REGISTRY.register()
class TiDabDDETR_C4Side2StreamOnCropsOimProtoConReFlip(TiDDETR_C4Side2StreamOnCropsOimProtoConReFlip):
    @classmethod
    def from_config(cls, cfg):
        ret = super().from_config(cfg)
        ret["det_head"]=DabDDetrDetHead(cfg,ret["backbone"].output_shape())
        return ret
@META_ARCH_REGISTRY.register()
class TiDDETR_C4Side2StreamOnCropsOimOnlineConReFlip(TiDDETR_C4Side2StreamOnCropsOimProtoConReFlip):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args,**kwargs)
    ...
    def crop_img_loss(self,img_embs,img_pids,crop_embs,crop_pids):
            ...
            return losses
@META_ARCH_REGISTRY.register()
class TiDDETR_NextC4Side2StreamOnCropsOimProtoConReFlip(TiDDETR_C4Side2StreamOnCropsOimProtoConReFlip):
    @configurable
    def __init__(
        self,
    ...
    def load_state_dict(self, *args, **kws):
        ...
        output = super(DDETR_Baseline,self).load_state_dict(*args, **kws)
        return output
    @classmethod
    def from_config(cls, cfg):
        ret = super().from_config(cfg)
...
@META_ARCH_REGISTRY.register()
class TiDDETR_NextC4Side2StreamOnCropsOimOnlineConReFlip(TiDDETR_NextC4Side2StreamOnCropsOimProtoConReFlip):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args,**kwargs)
        self.contrast_loss=SupConLoss()
...
@META_ARCH_REGISTRY.register()
class TiDDETR_NextC4Side2StreamOnCropsOimProtoConReFlipTrip(TiDDETR_NextC4Side2StreamOnCropsOimProtoConReFlip):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args,**kwargs)
        self.triplet_loss = TripletLoss(0.3, "mean")
...
@META_ARCH_REGISTRY.register()
class TiDDETR_NextC4SideSep2StreamOnCropsOimProtoConReFlipTrip(TiDDETR_NextC4Side2StreamOnCropsOimProtoConReFlipTrip):
    def get_reid_backbone_features(self, det_backbone_features, image_list):
        del image_list
        reid_res3_feat = (
            checkpoint.checkpoint(self.side_res3, det_backbone_features["stage1_unorm"])
