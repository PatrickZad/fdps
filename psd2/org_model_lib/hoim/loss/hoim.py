...
class OIM2CQ(autograd.Function):
    @staticmethod
    def forward(
        ctx, inputs, targets, lut, cq, cqb, cq_omega, cqb_omega, omega_decay, momentum
    ):
        ...
        omega_x_background = torch.cat([outputs_labeled, outputs_unlabeled], dim=1).max(
            dim=1
        )[0] / (outputs_background.max(dim=1)[0] + 1e-12)
        ...
        return torch.cat(
            [outputs_background, outputs_labeled, outputs_unlabeled], dim=1
        )
    ...
    def backward(ctx, grad_outputs):
        ...
        inputs, targets, omega_x_unlabeled, omega_x_background = tensor_gather(
            [inputs, targets, omega_x_unlabeled, omega_x_background]
        )

        grad_inputs = None
        if ctx.needs_input_grad[0]:
            grad_inputs = grad_outputs.mm(torch.cat([cqb, lut, cq], dim=0))
            if grad_inputs.dtype == torch.float16:
                grad_inputs = grad_inputs.to(torch.float32)

        for x, y, omega, omega_b in zip(
            inputs, targets, omega_x_unlabeled, omega_x_background
        ):
            if y == -1:
...
def oim2cq(
    inputs, targets, lut, cq, cqb, cq_omega, cqb_omega, omega_decay=0.99, momentum=0.5
):
    return OIM2CQ.apply(
        inputs,
        targets,
...
class HOIMLoss(nn.Module):
    def __init__(
        ...
        self.register_buffer("cq_omega", torch.zeros(self.num_unlabeled))

        self.register_buffer("cqb", torch.zeros(self.num_background, self.num_features))
        self.register_buffer("cqb_omega", torch.zeros(self.num_background))
    ...
    def build_det_loss(self, all_probs, roi_label=None):
        ...
        if self.training:
            focal_score = (
                self.alpha_d * (1 - cls_score) ** self.gamma_d * cls_score.log()
            )
    ...
    def forward(self, inputs, roi_label=None):
        ...
        if self.training:
            inds = label >= 0
            label = label[inds]
            projected_non_bg = projected[inds.unsqueeze(1).expand_as(projected)].view(
                -1, (self.num_background + self.num_pids + self.num_unlabeled)
            )
            ...
            loss_oim = F.nll_loss(focal_p_i, label, reduction="none", ignore_index=5554)
