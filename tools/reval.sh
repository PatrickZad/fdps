...
!
#prw
for dir_post in coseg-oim-4head-gem coseg-oim-4head-gem_t fc-oim-4head-gem fc-oim-4head-gem_t fc-oim-4head-gem_tapk
do
    out_dir=outputs/refine/nova/spps-prw_msroi${dir_post}
    python tools/train_spps_baseline.py --config-file ${out_dir}/config.yaml \
    --num-gpus 2 --resume --eval-only --dist-url tcp://127.0.0.1:60888 SOLVER.IMS_PER_BATCH 10 \
    MODEL.WEIGHTS ${out_dir}/model_0038759.pth \
    TEST.IMS_PER_PROC 5 \
...
for dir_post in coseg-oim-4head-gem_tapk
do
    out_dir=outputs_expd/refine/nova/spps-prw_msroi${dir_post}
    python tools/train_spps_baseline.py --config-file ${out_dir}/config.yaml \
    --num-gpus 2 --resume --eval-only --dist-url tcp://127.0.0.1:60888 SOLVER.IMS_PER_BATCH 10 \
    MODEL.WEIGHTS ${out_dir}/model_0038759.pth \
    TEST.IMS_PER_PROC 5 \
