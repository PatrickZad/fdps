...
from fdps.structures import Boxes, Instances, pairwise_iou
...
class CascadeROIHeads(StandardROIHeads):
    ...
    def forward(self, images, features, proposals, targets=None):
        ...
        if self.training:
            ...
            pred_instances, losses = self._forward_box(
                features, proposals, targets
            )  # NOTE change to return pred instances
    ...
    def _forward_box(self, features, proposals, targets=None):
        ...
        if self.training:
            ...
            # visualize only the last head
            with torch.no_grad():
                scores = head_outputs[-1][0].predict_probs(
                    head_outputs[-1][1], head_outputs[-1][2]
                )
