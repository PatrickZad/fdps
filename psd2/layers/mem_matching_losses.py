...
class MemClsLoss(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.match_layer = layers_map[match_layer](mem_len, feat_len, sync)
    ...
    def forward(self, pfeats, mids, update_mms):
        """
        pids: index in memory
        """
        if self.normalize:
            pfeats = F.normalize(pfeats, dim=-1)
        matching_scores = self.match_layer(pfeats, mids, update_mms) * self.match_factor
        pid_labels = mids.clone()
        pid_labels[pid_labels < 0] = -1
        n_inst_feats = (pid_labels > -1).sum()
        if matching_scores.shape[0] == 0:
            loss_matching = matching_scores.new_tensor([0])
        else:
            if self.use_focal:
                pass
            else:
                pass
        comm.synchronize()
        all_num_inst = comm.all_gather(n_inst_feats)
        num_inst = sum([num.to("cpu") for num in all_num_inst])
        num_inst = torch.clamp(num_inst / comm.get_world_size(), min=1).item()
        loss_val = loss_matching.sum() / num_inst
        return {"loss_matching": loss_val * self.loss_weights["loss_matching"]}
