...
from .faster_rcnn_norm_aware import (
...
class PixelWiseNormAwareEmbeddingProj(NormAwareEmbeddingProj):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for ftname, in_chennel, indv_dim in zip(
            ...
            proj = nn.Sequential(
                nn.Conv2d(in_chennel, indv_dim, kernel_size=1), nn.BatchNorm2d(indv_dim)
            )
...
def get_pixel_wise_norm_aware_model(args, training=True, pretrained_backbone=True):
    ...
    backbone, conv_head = resnet_backbone(
        "resnet50", pretrained_backbone, return_res4=return_res4, GAP=False
    )
    ...
    coord_fc = CoordRegressor(2048, num_classes=2, RCNN_bbox_bn=args.rcnn_bbox_bn)
    ...
    if args.smr:
        reid_loss = OIMLossSMR(
            args.num_features,
            args.num_pids,
            args.num_cq_size,
            args.train.oim_momentum,
            args.oim_scalar,
            args.smr_omega_decay,
        )
    else:
    ...
    model = net_fn(
        ...
        min_size=phase_args.min_size,
        max_size=phase_args.max_size,
