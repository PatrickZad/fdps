...
@META_ARCH_REGISTRY.register()
class TiRCNN_C4SideEmbed(TiRCNN_C4Side):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args,**kwargs)
        self.pfeat_pooling_1=copy.deepcopy(self.pfeat_pooling)
        self.embed=nn.ModuleList()
        for in_channel in [1024,2048]:
            proj = nn.Sequential(nn.Linear(in_channel, 1024), nn.BatchNorm1d(1024))
            init.normal_(proj[0].weight, std=0.01)
            init.normal_(proj[1].weight, std=0.01)
            init.constant_(proj[0].bias, 0)
            init.constant_(proj[1].bias, 0)
            self.embed.append(proj)
    def get_reid_person_features(self, reid_backbone_features, person_boxes):
        d2_boxes = [Boxes(pi_boxes) for pi_boxes in person_boxes]
        roi_feats_res4 = self.reid_pooler([reid_backbone_features], d2_boxes)
        roi_feats_res5 = (
            checkpoint.checkpoint(self.side_res5, roi_feats_res4)
            if self.use_checkpoint
            else self.side_res5(roi_feats_res4)
        )  # n x c x h x w
        return (roi_feats_res4,roi_feats_res5)
    def get_reid_embed(self, p_feat_maps):
        res4,res5=p_feat_maps
        pfeat_embs_4 = self.pfeat_pooling(res4).view(res4.shape[0],res4.shape[1] )
        pfeat_embs_5=self.pfeat_pooling_1(res5).view(res5.shape[0],res5.shape[1] )
        pfeat_embs_4=self.embed[0](pfeat_embs_4)
        pfeat_embs_5=self.embed[1](pfeat_embs_5)
        pfeat_embs=torch.cat([pfeat_embs_4,pfeat_embs_5],dim=-1)
        return tF.normalize(pfeat_embs, dim=-1)

@META_ARCH_REGISTRY.register()
class TiRCNN_C4Side_XDet(TiRCNN_C4Side):
    @configurable
    def __init__(self,*args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.pred_rst = None
    def forward(self, input_list):
        assert not self.training
        assert self.train_task != "det"
        if "query" in input_list[0]:
            return self.inf_query(input_list)
        if self.pred_rst is None:
            pred_rst = torch.load(
                "Data/model_zoo/simfpnlite_cuhk_gallery_gt_inf.pt",
        ...
        # print("det backbone time: {}".format(time.time() - t0))
        det_pred = {}
        det_pred["pred_boxes"] = [self.pred_rst[bi._image_id][:, :4] for bi in gt_instances]
        det_pred["pred_scores"] = [
            self.pred_rst[bi._image_id][:, 4]
            for bi in gt_instances
        ]
        for i, gt_i in enumerate(gt_instances):
            # back to org scale
            org_h, org_w = gt_i._org_hw
            h, w = gt_i._image_size
            factor = torch.tensor(
                [[w / org_w, h / org_h, w / org_w, h / org_h]],
                dtype=torch.float32,
                device=self.device,
            )
            det_pred["pred_boxes"][i] = det_pred["pred_boxes"][i] * factor
        reid_feats = self.forward_ps(det_pred, det_bk_features, images, gt_instances)
        del det_bk_features
        outputs = {"pred_boxes": [], "pred_scores": [], "reid_feats": []}
        for pi, pfeats in enumerate(reid_feats):
            boxes = det_pred["pred_boxes"][pi]
            ...
            scores = det_pred["pred_scores"][pi].unsqueeze(-1)
            outputs["pred_boxes"].append(org_boxes)
            outputs["pred_scores"].append(scores)
            outputs["reid_feats"].append(pfeats)
        return outputs
...
class TiRCNN_C4HeadJoint(TiRCNN_C4SideJoint):
    ...
    def load_state_dict(self, *args, **kws):
        ...
        return output
@META_ARCH_REGISTRY.register()
class TiRCNN_C4SideD3(TiRCNN_C4Side):
    @configurable
    def __init__(self, bn_neck, side_init, **kwargs) -> None:
        super(TiRCNN_C4Side, self).__init__(**kwargs)
...
@META_ARCH_REGISTRY.register()
class TiRCNN_C4SideTrip2(TiRCNN_C4Side):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.triplet_loss = TripletLoss(0.3, "mean")
    ...
    def forward_ps(self, det_pred, det_backbone_features, image_list, gts):
        ...
        if self.training:
            ...
            feats1 = pos_embs[pos_ids > -1]
            feats2 = torch.cat([pos_embs, oim_lookup], dim=0)
            trip = self.triplet_loss(
                feats1,
                feats2,
                pos_ids[pos_ids > -1],
                torch.cat([pos_ids, lookup_ids], dim=0),
                normalize_feature=True,
            )
            for k, v in trip.items():
                if "loss" in k:
...
@META_ARCH_REGISTRY.register()
class TiRCNN_C4SideTrip1Ms1(TiRCNN_C4SideTrip1):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.side_res5 = nn.Sequential(
...
@META_ARCH_REGISTRY.register()
class TiRCNN_C4SideTrip1Ms2(TiRCNN_C4SideTrip1Ms1):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.output3 = Conv2d(
...
@META_ARCH_REGISTRY.register()
class TiRCNN_C4SideTrip1_FixA(TiRCNN_C4SideTrip1):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        del self.alpha_res3,self.alpha_res4
        self.alpha_res3 = (torch.tensor(0.5)/1-torch.tensor(0.5)).log()
        self.alpha_res4 = (torch.tensor(0.5)/1-torch.tensor(0.5)).log()
@META_ARCH_REGISTRY.register()
class TiRCNN_C4SideTrip1_2Net(TiRCNN_C4SideTrip1):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.side_stem=copy.deepcopy(self.backbone.stem)
    ...
    def load_state_dict(self, *args, **kws):
        output = super(TiRCNN_C4SideTrip1,self).load_state_dict(*args, **kws)
        model_resume = True
        for param_k in output.missing_keys:
            if "side_res" in param_k:
                model_resume = False
                ...
                for pn, pa in state_dict.items():
                    if res_name in pn:
                        ...
                        if not isinstance(pa, torch.Tensor):
                            pa = torch.from_numpy(pa).to(self.device)
                        bn_neck_params[pn.split("bn_neck")[1][1:]] = pa
                    elif "stem" in pn and "fc" not in pn:
                        if not isinstance(pa, torch.Tensor):
                            pa = torch.from_numpy(pa).to(self.device)
                        stem_params[pn.split("stem")[1][1:]] = pa
            self.side_stem.load_state_dict(stem_params)
        ...
        return output
    def get_reid_backbone_features(self, det_backbone_features, image_list):
        del det_backbone_features
        reid_feat=self.side_stem(image_list.tensor)
        del image_list
        reid_feat=self.side_res2(reid_feat)
        reid_feat=self.side_res3(reid_feat)
        reid_feat=self.side_res4(reid_feat)
        return reid_feat
...
@META_ARCH_REGISTRY.register()
class TiRCNNFPN_Baseline(TiRCNN_Baseline):
    def inf_query(self, input_list):
        images, gt_instances = self.preprocess_input(qd["query"] for qd in input_list)
    ...
    def forward_det(self, image_list, features, gt_instances):
        if self.training and self.train_task == "ps":
            if not self.train_with_det:
                return {
                    "pred_boxes": [],
                    "pred_scores": [],
            ...
            return super().forward_det(image_list, features, gt_instances)
...
@META_ARCH_REGISTRY.register()
class TiRCNNFPN_C4Side(TiRCNNFPN_Baseline, TiRCNN_C4Side):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        TiRCNN_C4Side.__init__(self, *args, **kwargs)

    @classmethod
    def from_config(cls, cfg):
        return TiRCNN_C4Side.from_config(cfg)

    def load_state_dict(self, *args, **kws):
        return TiRCNN_C4Side.load_state_dict(self, *args, **kws)

    def get_reid_backbone_features(self, det_backbone_features, image_list):
        return TiRCNN_C4Side.get_reid_backbone_features(
            self, det_backbone_features, image_list
        )

    def get_reid_person_features(self, reid_backbone_features, person_boxes):
        return TiRCNN_C4Side.get_reid_person_features(
            self, reid_backbone_features, person_boxes
        )

    def get_reid_embed(self, p_feat_maps):
        return TiRCNN_C4Side.get_reid_embed(self, p_feat_maps)


@META_ARCH_REGISTRY.register()
class TiRCNNFPN_C4SideTrip1(TiRCNNFPN_Baseline, TiRCNN_C4SideTrip1):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        TiRCNN_C4SideTrip1.__init__(self, *args, **kwargs)

    @classmethod
    def from_config(cls, cfg):
        return TiRCNN_C4SideTrip1.from_config(cfg)

    def load_state_dict(self, *args, **kws):
        return TiRCNN_C4SideTrip1.load_state_dict(self, *args, **kws)

    def forward_ps(self, *args, **kws):
        return TiRCNN_C4SideTrip1.forward_ps(self, *args, **kws)

    def get_reid_backbone_features(self, det_backbone_features, image_list):
        return TiRCNN_C4SideTrip1.get_reid_backbone_features(
            self, det_backbone_features, image_list
        )

    def get_reid_person_features(self, reid_backbone_features, person_boxes):
        return TiRCNN_C4SideTrip1.get_reid_person_features(
            self, reid_backbone_features, person_boxes
        )

    def get_reid_embed(self, p_feat_maps):
        return TiRCNN_C4SideTrip1.get_reid_embed(self, p_feat_maps)


@META_ARCH_REGISTRY.register()
class TiRCNNFPN_C4SideD3(TiRCNNFPN_Baseline, TiRCNN_C4SideD3):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        TiRCNN_C4SideD3.__init__(self, *args, **kwargs)

    @classmethod
    def from_config(cls, cfg):
        return TiRCNN_C4SideD3.from_config(cfg)

    def load_state_dict(self, *args, **kws):
        return TiRCNN_C4SideD3.load_state_dict(self, *args, **kws)

    def get_reid_backbone_features(self, det_backbone_features, image_list):
        return TiRCNN_C4SideD3.get_reid_backbone_features(
            self, det_backbone_features, image_list
        )

    def get_reid_person_features(self, reid_backbone_features, person_boxes):
        return TiRCNN_C4SideD3.get_reid_person_features(
            self, reid_backbone_features, person_boxes
        )

    def get_reid_embed(self, p_feat_maps):
        return TiRCNN_C4SideD3.get_reid_embed(self, p_feat_maps)
...
"""
@META_ARCH_REGISTRY.register()
class TiRCNN_Os4Ladder2(TiRCNN_Baseline):
    @configurable
    def __init__(self, bn_neck, side_init, **kwargs) -> None:
        super().__init__(**kwargs)
        ...
        # side reid network
        self.bn_neck = bn_neck
        self.side_conv2 = OSNet._make_layer(
            block=OSBlock,
            layer=2,
            in_channels=64,
            out_channels=256,
        ...
        # init side network
        for osm in [self.side_conv2, self.side_conv3, self.side_conv4, self.side_conv5]:
    ...
    def load_state_dict(self, *args, **kws):
        ...
        if not model_resume and self.side_init != "":
            ...
            self.side_conv2.load_state_dict(side_conv_params[0], strict=False)
            self.side_conv3.load_state_dict(side_conv_params[1])
            self.side_conv4.load_state_dict(side_conv_params[2], strict=False)
            self.side_conv5.load_state_dict(side_conv_params[3])

        return output

    def get_reid_backbone_features(self, det_backbone_features, image_list):
        del image_list
        reid_conv2_feat = self.side_conv2(det_backbone_features["stem"])

        reid_conv3_feat = self.side_conv3(
            self.lateral_conv2(det_backbone_features["res2"]) + reid_conv2_feat
        )
...
@META_ARCH_REGISTRY.register()
class TiRCNN_Os4Ladder2(TiRCNN_Baseline):
    # NOTE TiRCNN_Os4Ladder3 actually
    @configurable
    def __init__(self, bn_neck, side_init, **kwargs) -> None:
        super().__init__(**kwargs)
    ...
    def get_reid_backbone_features(self, det_backbone_features, image_list):
        ...
        reid_conv3_feat = self.side_conv3(det_backbone_features["res2"])
...
@META_ARCH_REGISTRY.register()
class TiRCNN_R34C4Ladder3(TiRCNN_Baseline):
    @configurable
    def __init__(self, bn_neck, side_init, **kwargs) -> None:
        super().__init__(**kwargs)
...
@META_ARCH_REGISTRY.register()
class TiRCNN_R34C4Side(TiRCNN_C4Side):
    @configurable
    def __init__(self, bn_neck, side_init, **kwargs) -> None:
        super(TiRCNN_C4Side, self).__init__(**kwargs)
...
@META_ARCH_REGISTRY.register()
class TiRCNN_R34C4SideTrip1(TiRCNN_R34C4Side):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.triplet_loss = TripletLoss(0.3, "mean")
...
@META_ARCH_REGISTRY.register()
class TiRCNN_R50C4Ladder3(TiRCNN_C4Side):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(**kwargs)
        bk_out_shape = self.backbone.output_shape()
                ...
                for pn, pa in state_dict.items():
                    if res_name in pn:
                        ...
                        if not isinstance(pa, torch.Tensor):
                            pa = torch.from_numpy(pa).to(self.device)
                        bn_neck_params[pn.split("bn_neck")[1][1:]] = pa
                    elif "map_channel" in pn:
                        if not isinstance(pa, torch.Tensor):
                            pa = torch.from_numpy(pa).to(self.device)
                        lateral_2_params[pn.split("map_channel")[1][1:]] = pa
            self.side_res3.load_state_dict(side_res_params[0])
            self.side_res4.load_state_dict(side_res_params[1])
            self.side_res5.load_state_dict(side_res_params[2])
            if len(bn_neck_params) > 0:
...
@META_ARCH_REGISTRY.register()
class DnTiRCNN_C4Side(TiRCNN_C4Side):
    @configurable
    def __init__(self, dn_head_src, **kwargs) -> None:
        super().__init__(**kwargs)
        self.dn_head_src = dn_head_src
        res5_block = nn.Sequential(
            *ResNet.make_stage(
                **{
                    "num_blocks": 3,
                    "stride_per_block": [2, 1, 1],
        ...
        self.dn_head = DnRes5Head(
            pooler=copy.deepcopy(self.reid_pooler),
            res5=res5_block,
            box_predictor=copy.deepcopy(self.roi_heads.box_predictor),
            proposal_matcher=copy.deepcopy(self.roi_heads.proposal_matcher),
        )
        for param in self.dn_head.parameters():
            param.requires_grad_(True)
        self.dn_head.box_predictor.cls_score.requires_grad_(False)

    @classmethod
    def from_config(cls, cfg):
        res = super().from_config(cfg)
        res["dn_head_src"] = cfg.REID_HEAD.DN_HEAD_INIT
        return res
    ...
    def load_state_dict(self, *args, **kws):
        ...
        if not model_resume:
            # model is not resuming from saved checkpoint
            # init dn head
            if self.dn_head_src == "det":
                with torch.no_grad():
                    self.dn_head.load_state_dict(self.roi_heads.state_dict())
            elif self.dn_head_src == "ps":
                param_dict_ps = self.side_res5.state_dict()
                with torch.no_grad():
                    self.dn_head.res5.load_state_dict(param_dict_ps)
            else:
                assert self.dn_head_src == "share"
                self.dn_head.res5 = self.side_res5
    ...
    def forward_ps(self, det_pred, det_backbone_features, image_list, gts):
        ...
        if self.training:
            ...
            # dn losses
            dn_proposal_ts = []
            for boxes, scores, img_size in zip(
                pos_boxes, pos_scores, image_list.image_sizes
            ):
                p_instance = Instances(img_size)
                p_instance.proposal_boxes = Boxes(boxes)
                p_instance.objectness_logits = scores
                dn_proposal_ts.append(p_instance)
            dn_losses = self.dn_head(image_list, reid_bk_feats, dn_proposal_ts, gts)
            del reid_bk_feats
