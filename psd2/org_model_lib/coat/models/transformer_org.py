...
class Transformers(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if (
            self.training
            and self.use_feature_mask
            and self.feature_aug_type == "exchange_patch"
        ):
            feature_mask = exchange_patch(
                self.mask_para[0], self.mask_para[1], self.mask_para[2]
            )
