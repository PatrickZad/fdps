...
class Trainer(DefaultTrainer):
    ...
    def __init__(self, cfg, model_find_unused_parameters=True):
        ...
        self.labeling_period = cfg.REID_HEAD.PSEUDO_LABELING.CLUSTERING.PERIOD
    ...
    def build_clustering_loader(self, cfg):
        from psd2.data.catalog import MapperCatalog
        from psd2.data.build import get_detection_dataset_dicts, trivial_batch_collator
        from psd2.data.common import DatasetFromList, MapDataset
        from psd2.data.samplers import InferenceSampler
        ...
        batch_sampler = torchdata.sampler.BatchSampler(
            sampler,
            cfg.REID_HEAD.PSEUDO_LABELINGDATALOADER.MS_PER_PROC,
            drop_last=False,
        )
        data_loader = torchdata.DataLoader(
            dataset,
    ...
    def run_step(self):
        ...
        if self.iter % self.labeling_period == 0:
            data_loader = self.build_clustering_loader(self.cfg)
            self._trainer.model.label_update(data_loader)
