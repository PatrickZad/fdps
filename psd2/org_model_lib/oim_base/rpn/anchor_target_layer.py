...
class AnchorTargetLayer(nn.Module):
    ...
    def forward(self, scores, gt_boxes, img_info):
        ...
        shifts = torch.stack(
            (shift_x.view(-1), shift_y.view(-1), shift_x.view(-1), shift_y.view(-1)),
            dim=1,
        )
        ...
        inside_weights = inside_weights.view(1, height, width, A * 4).permute(
            0, 3, 1, 2
        )
        outside_weights = outside_weights.view(1, height, width, A * 4).permute(
            0, 3, 1, 2
        )
