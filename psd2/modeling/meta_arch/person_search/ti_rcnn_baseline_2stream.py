...
import copy
import time
from psd2.modeling.meta_arch.person_search import TiRCNN_Baseline,TiRCNN_Baseline_JointDc
...
class TiRCNN_2StreamOnCrops(TiRCNN_Baseline):
    @configurable
    ...
    def crop_img_loss(self,img_embs,img_pids,crop_embs,crop_pids):
        raise NotImplementedError
@META_ARCH_REGISTRY.register()
class TiRCNN_JointDc_2StreamOnCrops(TiRCNN_2StreamOnCrops,TiRCNN_Baseline_JointDc):
    def forward(self, input_list):
        return TiRCNN_Baseline_JointDc.forward(self,input_list)
    def forward_det(self, image_list, features, gt_instances):
        return TiRCNN_Baseline_JointDc.forward_det(self, image_list, features, gt_instances)
    def get_ps_pos_samples(self,gts):
        return TiRCNN_Baseline_JointDc.get_ps_pos_samples(self,gts)
    def forward_ps(self, det_backbone_features, image_list, gts,det_pred=None):
        ...
        if self.training:
            ...
            return p_embs
@META_ARCH_REGISTRY.register()
class TiRCNN_C4Side2StreamOnCrops(TiRCNN_2StreamOnCrops):
    @configurable
...
@META_ARCH_REGISTRY.register()
class TiRCNN_C4Side2StreamOnCrops_JointDc(TiRCNN_JointDc_2StreamOnCrops,TiRCNN_C4Side2StreamOnCrops):
    @configurable
    def __init__(self,bn_neck, side_init,*args, **kwargs) -> None:
        TiRCNN_JointDc_2StreamOnCrops.__init__(self,*args,**kwargs)
        self.side_init = side_init
        self.alpha_res3 = nn.Parameter(torch.tensor(0.0))
        self.alpha_res4 = nn.Parameter(torch.tensor(0.0))
        # side reid network
...
@META_ARCH_REGISTRY.register()
class TiRCNN_C4Side2StreamOnCropsOimProtoCon(TiRCNN_C4Side2StreamOnCrops):
    @configurable
...
@META_ARCH_REGISTRY.register()
class TiRCNN_C4Side2StreamOnCropsOimHybridCon(TiRCNN_C4Side2StreamOnCrops):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args,**kwargs)
        self.crop_loss=copy.deepcopy(self.reid_loss)
...
@META_ARCH_REGISTRY.register()
class TiRCNN_C4Side2StreamOnCropsOimHybridDetachCon(TiRCNN_C4Side2StreamOnCrops):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args,**kwargs)
        self.crop_loss=copy.deepcopy(self.reid_loss)
...
@META_ARCH_REGISTRY.register()
class TiRCNN_C4Side2StreamOnCropsOimProtoConRe(TiRCNN_C4Side2StreamOnCropsOimProtoCon):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args,**kwargs)
...
@META_ARCH_REGISTRY.register()
class TiRCNN_C4Side2StreamOnCropsOimProtoConReFlip(TiRCNN_C4Side2StreamOnCropsOimProtoConRe):
    def get_crops(self,image_list,boxes):
        tgt_size=(256,128)
        crops=[]
        for bi,boxesi in enumerate(boxes):
...
class TiRCNN_C4Side2StreamOnCropsOimProtoConReFlipDetBox(TiRCNN_C4Side2StreamOnCropsOimProtoConReFlip):
    def forward_ps(self, det_pred, det_backbone_features, image_list, gts):
        ...
        if self.training:
            ...
            return super().forward_ps(self, det_pred, det_backbone_features, image_list, gts)

@META_ARCH_REGISTRY.register()
class TiRCNN_C4Side2StreamOnCropsOimProtoConReFlipSide4(TiRCNN_C4Side2StreamOnCropsOimProtoConReFlip):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args,**kwargs)
                ...
                for pn, pa in state_dict.items():
                    if res_name in pn:
                        ...
                        if not isinstance(pa, torch.Tensor):
                            pa = torch.from_numpy(pa).to(self.device)
                        bn_neck_params[pn.split("bn_neck")[1][1:]] = pa
            self.side_res2.load_state_dict(side_res_params[0],strict=False)
            self.side_res3.load_state_dict(side_res_params[1],strict=False)
            self.side_res4.load_state_dict(side_res_params[2],strict=False)
            self.side_res5.load_state_dict(side_res_params[3],strict=False)
            if len(bn_neck_params) > 0:
                self.bn_neck.load_state_dict(bn_neck_params)
        return output
    ...
    def get_reid_backbone_features(self, det_backbone_features, image_list):
        del image_list
        reid_res2_feat = (
            checkpoint.checkpoint(self.side_res2, det_backbone_features["stem"])
            if self.use_checkpoint
            else self.side_res2(det_backbone_features["stem"])
        )
        ...
        reid_res3_feat = (
            checkpoint.checkpoint(self.side_res3, reid_res2_feat)
            if self.use_checkpoint
            else self.side_res3(reid_res2_feat)
        )
        del reid_res2_feat
        alpha = torch.sigmoid(self.alpha_res3)
        reid_res3_feat = (
        ...
        return reid_res4_feat
@META_ARCH_REGISTRY.register()
class TiRCNN_C4Side2StreamOnCropsOimProtoConReFlipSide2(TiRCNN_C4Side2StreamOnCropsOimProtoConReFlip):
    
    def get_reid_backbone_features(self, det_backbone_features, image_list):
        del image_list
        reid_res3_feat = det_backbone_features["res3"]
        reid_res4_feat = (
            checkpoint.checkpoint(self.side_res4, reid_res3_feat)
            if self.use_checkpoint
            else self.side_res4(reid_res3_feat)
        ...
        return reid_res4_feat
@META_ARCH_REGISTRY.register()
class TiRCNN_C4Side2StreamOnCropsOimProtoConReFlipSide1(TiRCNN_C4Side2StreamOnCropsOimProtoConReFlip):
    
    def get_reid_backbone_features(self, det_backbone_features, image_list):
        return det_backbone_features["res4"]
@META_ARCH_REGISTRY.register()
class TiRCNN_C4SideSep2StreamOnCropsOimProtoConReFlip(TiRCNN_C4Side2StreamOnCropsOimProtoConReFlip):
    def get_reid_backbone_features(self, det_backbone_features, image_list):
        del image_list
...
@META_ARCH_REGISTRY.register()
class TiRCNN_C4Side2StreamOnCropsOimProtoConReFlip_JointDc(TiRCNN_C4Side2StreamOnCrops_JointDc):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args,**kwargs)
        self.re=RandomErasingAfterNorm(self.pixel_mean[:,0,0],self.pixel_std[:,0,0])
...
@META_ARCH_REGISTRY.register()
class TiRCNN_C4Side2StreamOnCropsOimProtoConReFlip_JointC(TiRCNN_C4Side2StreamOnCropsOimProtoConReFlip_JointDc):
    def get_reid_backbone_features(self, det_backbone_features, image_list):
        del image_list
    ...
    def get_crop_embs(self,crops):
        ...
        crop_feats=self.get_reid_backbone_features(crop_bk_feats,None)
        crop_feats=(
                    checkpoint.checkpoint(self.roi_heads.res5, crop_feats)
                    if self.use_checkpoint and self.training
                    else self.roi_heads.res5(crop_feats)
                )
        crop_feats=self.get_reid_embed(crop_feats)
        return crop_feats
    def get_reid_person_features(self, reid_backbone_features, person_boxes):
        ...
        roi_feats = self.reid_pooler([reid_backbone_features], d2_boxes)
        roi_feats = (
            checkpoint.checkpoint(self.roi_heads.res5, roi_feats)
            if self.use_checkpoint and self.training
            else self.roi_heads.res5(roi_feats)
        )  # n x c x h x w
        return roi_feats

from psd2.layers.metric_loss import TripletLoss
@META_ARCH_REGISTRY.register()
class TiRCNN_C4Side2StreamOnCropsOimProtoConReFlipTrip(TiRCNN_C4Side2StreamOnCropsOimProtoConReFlip):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args,**kwargs)
...
@META_ARCH_REGISTRY.register()
class TiRCNN_C4SideSep2StreamOnCropsOimProtoConReFlipTrip(TiRCNN_C4Side2StreamOnCropsOimProtoConReFlipTrip):
    def get_reid_backbone_features(self, det_backbone_features, image_list):
        del image_list
        reid_res3_feat = (
            checkpoint.checkpoint(self.side_res3, det_backbone_features["res2"])
...
@META_ARCH_REGISTRY.register()
class TiRCNN_C4Side2StreamOnCropsOimProtoConReFlipTrip_DC5(TiRCNN_C4Side2StreamOnCropsOimProtoConReFlipTrip):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args,**kwargs)
        self.side_res5 = nn.Sequential(
...
@META_ARCH_REGISTRY.register()
class TiRCNN_C4Side2StreamOnCropsOimProtoConReFlipTrip_JointDc(TiRCNN_C4Side2StreamOnCropsOimProtoConReFlip_JointDc):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args,**kwargs)
        self.triplet_loss = TripletLoss(0.3, "mean")
...
@META_ARCH_REGISTRY.register()
class TiRCNN_C4Side2StreamOnCropsOimProtoConReFlip_Joint(TiRCNN_C4Side2StreamOnCropsOimProtoConReFlip_JointDc):
    def get_reid_backbone_features(self, det_backbone_features, image_list):
        del image_list
        reid_res3_feat = (
            checkpoint.checkpoint(self.side_res3, det_backbone_features["res2"])
...
@META_ARCH_REGISTRY.register()
class TiRCNN_C4Side2StreamOnCropsOimOnlineConReFlip(TiRCNN_C4Side2StreamOnCropsOimProtoConReFlip):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args,**kwargs)
    ...
    def crop_img_loss(self,img_embs,img_pids,crop_embs,crop_pids):
            ...
            if pos_mask.sum().item()==0:
                ...
                l_con=self.contrast_loss(crop_embs[pos_mask],img_embs,crop_pids[pos_mask],img_pids)*self.w_kd
                losses["loss_contrast_crop"]=l_con
                l_con=self.contrast_loss(img_embs[pos_mask],crop_embs,img_pids[pos_mask],crop_pids)*self.w_kd
                losses["loss_contrast_image"]=l_con
            return losses
@META_ARCH_REGISTRY.register()
class TiRCNN_NextC4Side2StreamOnCropsOimProtoConReFlip(TiRCNN_C4Side2StreamOnCropsOimProtoConReFlip):
    @configurable
    def __init__(
        self,
    ...
    def load_state_dict(self, *args, **kws):
        ...
        output = super(TiRCNN_Baseline,self).load_state_dict(*args, **kws)
        return output
    @classmethod
    def from_config(cls, cfg):
...
class TiRCNN_NextC4Side2StreamOnCropsOimOnlineConReFlip(TiRCNN_NextC4Side2StreamOnCropsOimProtoConReFlip):
    ...
    def crop_img_loss(self,img_embs,img_pids,crop_embs,crop_pids):
            ...
            return losses
@META_ARCH_REGISTRY.register()
class TiRCNN_NextC4SideSep2StreamOnCropsOimProtoConReFlip(TiRCNN_NextC4Side2StreamOnCropsOimProtoConReFlip):
    def get_reid_backbone_features(self, det_backbone_features, image_list):
        del image_list
        reid_res3_feat = self.side_res3(det_backbone_features["stage1_unorm"])
...
@META_ARCH_REGISTRY.register()
class TiRCNN_NextC4Side2StreamOnCropsOimProtoConReFlip_JointDc(TiRCNN_C4Side2StreamOnCropsOimProtoConReFlip_JointDc):
    @configurable
    def __init__(
        self,
        conv_next,**kwargs,) -> None:
...
@META_ARCH_REGISTRY.register()
class TiRCNN_NextC4Side2StreamOnCropsOimProtoConReFlipTrip(TiRCNN_NextC4Side2StreamOnCropsOimProtoConReFlip):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args,**kwargs)
        self.triplet_loss = TripletLoss(0.3, "mean")
...
@META_ARCH_REGISTRY.register()
class TiRCNN_NextC4SideSep2StreamOnCropsOimProtoConReFlipTrip(TiRCNN_NextC4Side2StreamOnCropsOimProtoConReFlipTrip):
    def get_reid_backbone_features(self, det_backbone_features, image_list):
        del image_list
        reid_res3_feat = (
            checkpoint.checkpoint(self.side_res3, det_backbone_features["stage1_unorm"])
...
@META_ARCH_REGISTRY.register()
class TiRCNN_NextC4Side2StreamOnCropsOimProtoConReFlipTrip_JointDc(TiRCNN_NextC4Side2StreamOnCropsOimProtoConReFlip_JointDc):
    @configurable
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args,**kwargs)
        self.triplet_loss = TripletLoss(0.3, "mean")
...
@META_ARCH_REGISTRY.register()
class TiRCNN_C4SideD2StreamOnCropsOimProtoConReFlip(TiRCNN_C4Side2StreamOnCropsOimProtoConReFlip):
    @configurable
